...
namespace PAPRECA{
	...
	void executeTrajectoryDurationCommand( std::vector< std::string > &commands , PaprecaConfig &papreca_config ){
		...
		if( papreca_config.getKMCperMD( ) == std::numeric_limits< unsigned long int >::max( ) ){ allAbortWithMessage( MPI_COMM_WORLD , "KMC_per_MD must be set before setting a trajectory duration." ); }
